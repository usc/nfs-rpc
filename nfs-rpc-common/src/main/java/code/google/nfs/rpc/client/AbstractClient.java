...
public abstract class AbstractClient implements Client {
    ...
    public Object invokeSync(String targetInstanceName, String methodName, String[] argTypes, Object[] args, int timeout, int codecType, int protocolType) throws Exception {
        byte[][] argTypeBytes = new byte[argTypes.length][];
        for (int i = 0; i < argTypes.length; i++) {
            argTypeBytes[i] = argTypes[i].getBytes();
        }
        RequestWrapper wrapper = new RequestWrapper(targetInstanceName.getBytes(), methodName.getBytes(), argTypeBytes, args, timeout, codecType, protocolType);
        return invokeSyncIntern(wrapper);
    }
    ...
    private Object invokeSyncIntern(RequestWrapper wrapper) throws Exception {
        ...
        Object result = null;
        try {
            result = responseQueue.poll(wrapper.getTimeout() - (System.currentTimeMillis() - beginTime), TimeUnit.MILLISECONDS);
        } catch (Exception e) {
            responses.remove(wrapper.getId());
        ...
        if (PRINT_CONSUME_MINTIME > 0 && isWarnEnabled) {
            ...
            if (consumeTime > PRINT_CONSUME_MINTIME) {
                LOGGER.warn("client.invokeSync consume time: " + consumeTime + " ms, server is: " + getServerIP() + ":" + getServerPort() + " request id is:" + wrapper.getId());
            }
        }
        if (result == null) {
            String errorMsg = "receive response timeout(" + wrapper.getTimeout() + " ms),server is: " + getServerIP() + ":" + getServerPort() + " request id is:" + wrapper.getId();
        ...
        try {
            ...
            if (responseWrapper.getResponse() instanceof byte[]) {
                ...
                if (((byte[]) responseWrapper.getResponse()).length == 0) {
                    ...
                    Object responseObject = Codecs.getDecoder(responseWrapper.getCodecType()).decode(responseClassName, (byte[]) responseWrapper.getResponse());
        ...
        if (responseWrapper.isError()) {
            ...
            t.fillInStackTrace();
            String errorMsg = "server error,server is: " + getServerIP() + ":" + getServerPort() + " request id is:" + wrapper.getId();
