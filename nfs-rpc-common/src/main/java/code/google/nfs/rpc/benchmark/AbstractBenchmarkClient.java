...
public abstract class AbstractBenchmarkClient {
    ...
    public void run(String[] args) throws Exception {
        if (args == null || (args.length != 7 && args.length != 8)) {
            throw new IllegalArgumentException("must give seven or eight args, serverIP serverPort concurrents timeout codectype requestSize runtime(seconds) clientNums");
        }
        ...
        for (int i = 0; i < concurrents; i++) {
            ClientRunnable runnable = getClientRunnable(serverIP, serverPort, clientNums, timeout, codectype, requestSize, barrier, latch, endtime, benchmarkBeginTime);
        ...
        System.out.println(" Requests: " + allRequest + " Success: " + (allRequestSum + ignoreRequest) * 100 / allRequest + "% (" + (allRequestSum + ignoreRequest) + ") Error: " + (allErrorRequestSum + ignoreErrorRequest) * 100 / allRequest + "% (" + (allErrorRequestSum + ignoreErrorRequest) + ")");
        System.out.println(" Avg TPS: " + benchmarkRequest / times.keySet().size() + " Max TPS: " + maxTPS + " Min TPS: " + minTPS);
    ...
    public abstract ClientRunnable getClientRunnable(String targetIP, int targetPort, int clientNums, int rpcTimeout, int codecType, int requestSize, CyclicBarrier barrier, CountDownLatch latch, long endTime, long startTime);
